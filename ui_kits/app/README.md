...
This kit shows the product-surface variants of every primitive:

- **`.paste-zone`** — dashed-border paste box; takes `clay/lav/sage/sky/yellow` modifier to tint the border in the active tool's hue.
- **`.struct-card`** — Persona/Task/Format/Context card with a left accent bar in the tool hue.
- **`.result-block`** — the clean-prompt readout, on the warm cream card.
- **`.btn-primary` / `.btn-secondary` / `.btn-ghost`** — light-theme buttons. Primary is tinted per-tool by inline style.
- **`.chip` / `.chip.on`** — selectable pill (used for Tone, Library filters).
- **`.slider`** — Creative↔Precise input with a clay-to-sky gradient track.
- **`.lib-card`** — saved-prompt card in the library grid, with hue-tagged eyebrow.
...
- View it deployed via GitHub Pages at the repo's Pages URL.
